...
public class IPUtil {
	...
	protected IPUtil() {
		...
		b3 = new byte[3];
		try {
			String ipFilePath = IPUtil.class.getResource("/qqwry.dat").getFile();
		...
		for (long offset = ipBegin + 4; offset <= endOffset; offset += IP_RECORD_LENGTH) {
			...
			if (temp != -1) {
				...
				if (loc.country.indexOf(s) != -1 || loc.area.indexOf(s) != -1) {
					...
					// 得到起始IP
					readIP(offset - 4, b4);
					entry.beginIp = IPAnalysisUtils.getIpStringFromBytes(b4);
					// 得到结束IP
					readIP(temp, b4);
					entry.endIp = IPAnalysisUtils.getIpStringFromBytes(b4);
					// 添加该记录
					ret.add(entry);
				}
			}
	...
	public String getCountry(byte[] ip) {
		...
		if (ipFile == null)
			return "错误的IP数据库文件";
		// 保存ip，转换ip字节数组为字符串形式
		String ipStr = IPAnalysisUtils.getIpStringFromBytes(ip);
	...
	public String getCountry(String ip) {
		return getCountry(IPAnalysisUtils.getIpByteArrayFromString(ip));
	}
	...
	public String getArea(String ip) {
		return getArea(IPAnalysisUtils.getIpByteArrayFromString(ip));
	}
	 ...
	 * 
	 *  ip：要查询的IP
	 * beginIp：和被查询IP相比较的IP
	 * 返回相等返回0，ip大于beginIp则返回1，小于返回-1。
	 */
	...
	private String readString(long offset) {
		try {
			...
			for (i = 0, buf[i] = ipFile.readByte(); buf[i] != 0; buf[++i] = ipFile
				...
				;
			if (i != 0)
				return IPAnalysisUtils.getString(buf, 0, i, "GBK");
		} catch (IOException e) {
			System.out.println(e.getMessage());
		}
		return "";
	 ...
	 * 
	 */
	public static class IPAnalysisUtils {
		/**
		 * 从ip的字符串形式得到字节数组形式
		 * 
		 *  ip 字符串形式的ip
	...
	public List<String> getAllIp() {
		...
		for (long i = ipBegin; i < ipEnd; i += IP_RECORD_LENGTH) {
			try {
				...
				String ip = IPAnalysisUtils.getIpStringFromBytes(buf);
				list.add(ip);
			} catch (Exception e) {
				// ...
			}
